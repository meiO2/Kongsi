...
export default function OrderDetailView({ order: initialOrder }: { order: Order }) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-[720px] flex-col gap-6 px-4 pb-20 pt-8 sm:px-6">
      <div>
        <Link href="/umkm/pesanan" className="text-sm font-semibold text-[#3991FA] hover:underline">
          ← Kembali ke Pesanan
        </Link>
        <h1
      ...
      <section className="rounded-2xl border border-[#E4E1DF] bg-white p-5 sm:p-6">
        <h2 className="mb-3 text-sm font-semibold uppercase text-[#7A7876]">Customer</h2>
      ...
      <section className="rounded-2xl border border-[#E4E1DF] bg-white p-5 sm:p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold uppercase text-[#7A7876]">Status Pesanan</h2>
        ...
        {order.status === "menunggu" && (
          <p className="mt-4 text-sm text-[#7A7876]">
            Pesanan ini akan diproses secara otomatis setelah Kongsi mencapai target pembeli.
          </p>
        )}

        {order.status === "dibatalkan" && (
          <p className="mt-4 text-sm text-[#7A7876]">
            Kongsi tidak mencapai target. Pesanan dibatalkan dan dana dikembalikan ke pembeli.
          </p>
        )}
        ...
        {nextStep && (
          <ActionButton
            className="mt-4"
            onClick={() => setOrder((prev) => ({ ...prev, status: nextStep.nextStatus }))}
          >
      ...
      {/* Order detail */}
      <section className="rounded-2xl border border-[#E4E1DF] bg-white p-5 sm:p-6">
        <h2 className="mb-3 text-sm font-semibold uppercase text-[#7A7876]">Detail Pesanan</h2>
        <div className="flex flex-col divide-y divide-[#F1EFEF] text-sm">
          ...
          <div className="flex justify-between py-2.5">
            ...
            <span className="text-[#292828]">{formatRupiah(order.unitPrice)}</span>
          </div>
          <div className="flex justify-between py-2.5">
          ...
          {order.fulfillment === "delivery" && (
            <div className="flex justify-between py-2.5">
              ...
              <span className="text-[#292828]">{formatRupiah(order.deliveryFee ?? 0)}</span>
            </div>
          )}
      ...
      {/* Pemenuhan */}
      <section className="rounded-2xl border border-[#E4E1DF] bg-white p-5 sm:p-6">
        <h2 className="mb-3 text-sm font-semibold uppercase text-[#7A7876]">Pemenuhan</h2>
        {order.fulfillment === "pickup" ? (
          ...
          <div className="flex items-start gap-2 text-sm text-[#292828]">
            <span aria-hidden>🚚</span>
            <p>Pengiriman ke alamat customer</p>
          </div>
        )}
      </section>
    </main>
